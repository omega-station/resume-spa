...
const GitHub = (props: Props) => {
  const { isLeft, ariaLabel, color, fill, mode, url }: Props = { ...defaults, ...props };
    ...
    <StyledCorner className={`github-corner mode-${mode}`} isLeft={isLeft} isSmall={isSmall} aria-label={ariaLabel} color={color} fill={fill} href={url} mode={mode} target="_blank" rel="noopener noreferrer">
      {(mode === 'alternate' || mode === 'circle' || mode === 'square') && (
        <span>
          ...
          {mode === 'circle' && <FontAwesomeIcon icon={getIconProp('fab-github')} />}
          {mode === 'square' && <FontAwesomeIcon icon={getIconProp('fab-github-square')} />}
        </span>
      )}
      {mode === 'triangle' && (
